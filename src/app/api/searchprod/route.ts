...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (yahooFinanceUrl) {
      ...
      await page.goto(yahooFinanceUrl, { waitUntil: 'networkidle2', timeout: 30000 });
      ...
      const extractedData = await page.evaluate(() => {
        ...
        const coverTitle = document.querySelector('.cover-title')?.textContent?.trim() || '';
        console.log(coverTitle);

        // Extract data from all elements with class yf-1090901
        const yfClassElements = Array.from(document.querySelectorAll('.yf-1090901'));
        ...
        const regularTitle = document.querySelector('h1')?.textContent?.trim() || '';
        const date = document.querySelector('time')?.textContent?.trim() || '';
        const articleContent = Array.from(document.querySelectorAll('.caas-body p'))
      ...
      console.log(`Found ${extractedData.yfClassElements.length} elements with class yf-1090901`);
      
      let article = ""
      extractedData.yfClassElements.forEach((item) => {
        article = article+ item.text.substring(0, 100) + (item.text.length > 100);
      });
